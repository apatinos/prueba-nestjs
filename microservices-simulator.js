...
var DB = {
  ...
  actions: {
    ...
    updateNote: (data) => { DB.data.notes = DB.data.notes.map(note => { if(note.Id == data.IdId) {note.State = data.State; note.Title = data.Title;} return note; }); return DB.actions.getNote(data.Id); },
...
function mainBmsNotes() {
  ...
  const notesProtoPath = path.join(__dirname, 'notes.proto');
  const noteListsProtoPath = path.join(__dirname, 'note-lists.proto');
  const settingsProtoPath = path.join(__dirname, 'settings.proto');
